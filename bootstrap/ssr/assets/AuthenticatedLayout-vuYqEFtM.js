...
import { AlertCircle, AlertTriangle, Clock, MessageCircle, Bot, X, User, Send, Building2, Settings, UserPlus, KeyRound, Receipt, CreditCard, Wallet, FileText, Bell, MessageSquare, Calendar, Wrench, Users, GraduationCap, Search, ChevronRight, CheckCheck, Lock, Sparkles, Gift, ArrowRight, Loader2, LayoutDashboard, UserCog, DoorOpen, Package, CalendarCheck, Ticket, Briefcase, ShoppingBag, Megaphone, Siren, DollarSign, Handshake, Tag, LifeBuoy, ChartBar, Gauge, Server, Shield, Flag, Menu, Map, LogOut } from "lucide-react";
...
function NpsModalGestora({ activo }) {
  ...
  useEffect(() => {
    if (!activo) return;
    const t = setTimeout(() => {
      fetch("/nps/plataforma/estado", { headers: { Accept: "application/json" } }).then((r) => r.json()).then((d) => {
        if (d.deve_pedir) {
          setEstado(d);
          setAberto(true);
        }
  ...
  return /* @__PURE__ */ jsx("div", { className: "fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4", children: /* @__PURE__ */ jsxs("div", { className: "w-full max-w-md rounded-2xl bg-[#141428] border border-white/10 p-6", children: [
    /* @__PURE__ */ jsxs("div", { className: "flex items-start justify-between mb-4", children: [
      /* @__PURE__ */ jsx("h2", { className: "text-lg font-medium text-white", children: "A sua opinião conta" }),
      /* @__PURE__ */ jsx("button", { onClick: () => setAberto(false), className: "text-white/40 hover:text-white text-sm", children: "✕" })
    ] }),
    /* @__PURE__ */ jsx("p", { className: "text-sm text-white/80 mb-4", children: estado.pergunta }),
    /* @__PURE__ */ jsx("div", { className: "flex flex-wrap gap-2 mb-2", children: Array.from({ length: 11 }, (_, n) => /* @__PURE__ */ jsx(
      "button",
      {
        onClick: () => setNota(n),
        className: `w-9 h-9 rounded-lg text-sm font-semibold border transition-colors ${nota === n ? corNota(n) + " border-transparent" : "bg-white/5 text-white border-white/10 hover:bg-white/10"}`,
        children: n
      },
      n
    )) }),
    /* @__PURE__ */ jsxs("div", { className: "flex justify-between text-xs text-white/40 mb-4", children: [
    ...
    /* @__PURE__ */ jsx(
      "textarea",
      {
        ...
        className: "w-full px-3 py-2 mb-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder-white/30"
      }
    ),
    /* @__PURE__ */ jsx(
      "textarea",
      {
        ...
        rows: 2,
        placeholder: estado.seguimento,
        className: "w-full px-3 py-2 mb-4 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder-white/30"
      }
    ),
    /* @__PURE__ */ jsxs("div", { className: "flex gap-2 justify-end", children: [
      /* @__PURE__ */ jsx("button", { onClick: () => setAberto(false), className: "px-4 py-2 text-sm rounded-lg text-white/60 hover:text-white", children: "Agora não" }),
      /* @__PURE__ */ jsx(
        "button",
        {
          onClick: enviar,
          disabled: nota === null || aEnviar,
          className: "px-5 py-2 text-sm rounded-lg bg-cyan-500/20 text-cyan-200 hover:bg-cyan-500/30 disabled:opacity-50 font-medium",
          children: aEnviar ? "A enviar..." : "Enviar"
        }
      )
    ] })
  ] }) });
...
function PopupPublicidade() {
  const { popupPublicidade } = usePage().props;
  const popup = popupPublicidade ?? null;
  const [aberto, setAberto] = useState(false);
  useEffect(() => {
    if (!popup) return;
    const hoje = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    const chave = `popup_pub_${popup.id}_${hoje}`;
    try {
  ...
  function fechar() {
    if (popup) {
      const hoje = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
      try {
        localStorage.setItem(`popup_pub_${popup.id}_${hoje}`, "1");
      } catch {
      }
    }
    setAberto(false);
  ...
  return /* @__PURE__ */ jsx("div", { className: "fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4", onClick: fechar, children: /* @__PURE__ */ jsxs("div", { className: "relative w-full max-w-md overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl", onClick: (e) => e.stopPropagation(), children: [
    /* @__PURE__ */ jsx("button", { onClick: fechar, className: "absolute right-3 top-3 z-10 rounded-full bg-black/40 p-1.5 text-white hover:bg-black/60", children: /* @__PURE__ */ jsx(X, { className: "h-4 w-4" }) }),
    popup.imagem_url && /* @__PURE__ */ jsx("img", { src: popup.imagem_url, alt: popup.titulo, className: "h-48 w-full object-cover" }),
    /* @__PURE__ */ jsxs("div", { className: "p-6", children: [
      /* @__PURE__ */ jsx("h3", { className: "text-xl font-bold text-zinc-100", children: popup.titulo }),
      popup.mensagem && /* @__PURE__ */ jsx("p", { className: "mt-2 text-sm text-zinc-400", children: popup.mensagem }),
      popup.link_url && /* @__PURE__ */ jsx(
        "a",
        {
          href: popup.link_url,
          target: "_blank",
          ...
          onClick: fechar,
          className: "mt-4 block w-full rounded-lg bg-gradient-to-r from-cyan-500 to-purple-500 px-4 py-2.5 text-center font-semibold text-white hover:opacity-90",
          children: popup.botao_texto || "Saber mais"
        }
      )
    ] })
  ] }) });
...
function AuthenticatedLayout({ children }) {
    ...
    }] : [],
    {
      ...
      itens: [
        ...
        ...isSuperAdmin ? [
          ...
          { label: "Estado do Sistema", href: "/admin/sistema", icon: Server },
  ...
  return /* @__PURE__ */ jsxs("div", { className: "min-h-screen relative", children: [
    ...
    /* @__PURE__ */ jsx(NpsModalGestora, { activo: isGestoraNps }),
