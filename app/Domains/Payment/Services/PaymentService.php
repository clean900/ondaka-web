...
class PaymentService
{
    ...
    public function confirmarPagamento(
        ...
        // Se não foi passado user (ex: webhook automático), usar user sistema
        $adminUserId ??= (int) config('services.ondaka.system_user_id');

        if ($adminUserId < 1) {
            throw new \RuntimeException('User sistema (ONDAKA_SYSTEM_USER_ID) não configurado.');
        }
