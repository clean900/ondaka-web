...
class AdminFeaturesController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $subscriptions->getCollection()->transform(fn (FeatureSubscription $s) => [
            ...
            'owner' => [
                'tipo' => match ($s->owner_type) {
                    EmpresaGestora::class => 'empresa',
                    User::class => 'user',
                    default => 'condominio',
                },
                'id' => $s->owner_id,
                'nome' => $s->owner?->nome ?? $s->owner?->name ?? '—',
            ],
    ...
    public function show(FeatureSubscription $subscription): Response
    {
        ...
        return Inertia::render('Admin/Features/Show', [
            'subscription' => [
                ...
                'owner' => [
                    'tipo' => match ($subscription->owner_type) {
                        EmpresaGestora::class => 'empresa',
                        User::class => 'user',
                        default => 'condominio',
                    },
                    'id' => $subscription->owner_id,
                    'nome' => $subscription->owner?->nome ?? $subscription->owner?->name ?? '—',
    ...
    public function create(Request $request): Response
    {
        ...
        $condominos = User::query()
            ->whereNotNull('condominio_activo_id')
            ->leftJoin('condominios', 'users.condominio_activo_id', '=', 'condominios.id')
            ->orderBy('users.name')
            ->get([
                'users.id',
                'users.name',
        ...
        return Inertia::render('Admin/Features/Nova', [
            ...
            'condominos' => $condominos->map(fn ($u) => [
                'id' => $u->id,
                'nome' => $u->name,
                'email' => $u->email,
                'condominio_nome' => $u->condominio_nome,
            ]),
        ]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        $data = $request->validate([
            'tipo_owner' => 'required|in:empresa,condominio,user',
