...
const estadoConfig: Record<Estado, { label: string; color: string; Icon: any }> = {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-[#0a0a1a] text-zinc-100" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, \"Segoe UI\", system-ui, sans-serif" }}>
                ...
                {/* HERO */}
                <section className="relative overflow-hidden border-b border-blue-900/30 py-16">
                ...
                {/* FILTROS */}
                <section className="sticky top-[57px] z-40 border-b border-zinc-800 bg-[#0a0a1a]/95 backdrop-blur-md">
                    ...
                    <div className="mx-auto max-w-7xl px-6 space-y-12">
                        {filtradas.length === 0 ? (
                            <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-12 text-center text-zinc-500">
                                Nenhuma funcionalidade encontrada com estes filtros.
                            </div>
                        ) : (
                            filtradas.map((cat) => {
                                const Icon = cat.Icon;
                                ...
                                return (
                                    <div key={cat.id}>
                                        ...
                                        <div className="mb-6 flex items-center gap-4">
                                            <div className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${corClass.split(" ").slice(0, 2).join(" ")} ring-1 ${corClass.split(" ")[2]}`}>
                                                <Icon className={`h-7 w-7 ${corClass.split(" ")[3]}`} strokeWidth={1.5} />
                                            </div>
                                            <div className="flex-1">
                                                <h2 className="text-2xl font-bold">{cat.titulo}</h2>
                                                <p className="text-sm text-zinc-500">{cat.descricao}</p>
                                            </div>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="mb-2 flex items-start justify-between gap-2">
                                                            <h3 className="text-sm font-semibold leading-tight text-zinc-100">{f.nome}</h3>
                                                        </div>
                                                        <p className="mb-3 text-xs leading-relaxed text-zinc-400">{f.desc}</p>
                                                        <div className="flex items-center justify-between gap-2">
                                                            <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${conf.color}`}>
                                                                <StatusIcon className="h-3 w-3" />
                                                                {conf.label}
                                                            </span>
                                                            {f.quando && (
                                                                <span className="text-[10px] text-zinc-500">{f.quando}</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                );
                ...
                <section className="relative overflow-hidden py-20">
                    ...
                    <div className="relative mx-auto max-w-4xl px-6 text-center">
                        <h2 className="mb-4 text-3xl font-bold md:text-4xl">
                            Veja a ONDAKA em{" "}
                            <span className="bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">acção</span>
                        </h2>
                        <p className="mb-8 text-zinc-400">30 dias 100% grátis. Sem compromisso, cancele quando quiser.</p>
                        <Link
                            href="/registo"
